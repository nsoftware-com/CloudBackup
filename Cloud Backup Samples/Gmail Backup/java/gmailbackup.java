...
public class gmailbackup extends ConsoleDemo {
  ...
  public static void main(String[] args) {
    if (args.length < 6) {
      ...
      System.out.println("\nusage: gmailbackup -id client_id -secret client_secret -p path [-f filter] [-s YYYY/mm/DD] [-e YYYY/mm/DD] [-c max_connections] [-d]\n");
      ...
      System.out.println("Example: gmailbackup -id client_id -secret client_secret -p ../../test_folder\n");
      ...
      try {
        ...
        gmailbackup1.addGmailbackupEventListener(new DefaultGmailbackupEventListener() {
          public void afterMessageBackup(GmailbackupAfterMessageBackupEvent e) {
            synchronized (finishedMessageIds) {
              finishedMessageIds.add(e.id);
              System.out.println("Message backed up successfully. Progress: " + finishedMessageIds.size() + "/" + gmailbackup1.getMessageCount());
            }
          }
          public void beforeMessageBackup(GmailbackupBeforeMessageBackupEvent e) {
            if (e.skip) {
              System.out.println("Message exists locally, skipping: " + e.backupFile);
            }
          }
          public void endBackup(GmailbackupEndBackupEvent e) {
            backupCompleted = true;
            System.out.println("Backup Completed");
            System.out.println("Messages backed up: " + finishedMessageIds.size());
            System.out.println("Messages skipped: " + (gmailbackup1.getMessageCount() - finishedMessageIds.size()));
            if (gmailbackup1.isSyncDeletes()) {
              System.out.println("Messages deleted: " + messagesDeleted);
            }
          }
          public void log(GmailbackupLogEvent e) {
            System.out.println(e.message);
          }
          public void messageDelete(GmailbackupMessageDeleteEvent e) {
            messagesDeleted++;
            System.out.println("Message not present remotely, deleting local file: " + e.backupFile);
          }
          public void messageError(GmailbackupMessageErrorEvent e) {
            if (e.retry) {
        ...
        for (int i = 0; i < args.length; i++) {
          if (args[i].startsWith("-")) {
            if (args[i].toLowerCase().equals("-p")) {
              gmailbackup1.setDataFolder(args[i + 1]);
            }
            if (args[i].toLowerCase().equals("-f")) {
              gmailbackup1.setFilter(args[i + 1]);
            }
            if (args[i].toLowerCase().equals("-s")) {
              gmailbackup1.setStartDate(args[i + 1]);
            }
            if (args[i].toLowerCase().equals("-e")) {
              gmailbackup1.setEndDate(args[i + 1]);
            }
            if (args[i].toLowerCase().equals("-c")) {
              gmailbackup1.setMaxConnections(parseInt(args[i + 1]));
            }
            if (args[i].toLowerCase().equals("-d")) {
              gmailbackup1.setSyncDeletes(true);
            }
            if (args[i].toLowerCase().equals("-id")) {
              gmailbackup1.getOAuth().setClientId(args[i + 1]);
            }
            if (args[i].toLowerCase().equals("-secret")) {
              gmailbackup1.getOAuth().setClientSecret(args[i + 1]);
            }
          }
        }

        System.out.println("This is a basic demo showing how to backup your Gmail account.");
        ...
        gmailbackup1.startBackup();

        while (!backupCompleted) {
          gmailbackup1.doEvents();
        }
      } catch (Exception ex) {
